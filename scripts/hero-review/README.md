...
`measure-bundles.sh` runs the build and code checks. `check-journey.mjs` checks finite/reversible poses, camera and look-target continuity, exported orthonormal frames, wrap timing and the sphere crossing. `check-motion.mjs` checks the damped integrator.
