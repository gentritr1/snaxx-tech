...
spline=json.loads((OUT/'journey-spline.json').read_text());points=spline['points'];tree=KDTree(len(points))
...
remove=[f for f in bm.faces if tree.find(f.calc_center_median())[1]/(len(points)-1)>min(.3,spline['wrapEndT'])]
bmesh.ops.delete(bm,geom=remove,context='FACES');bm.to_mesh(thread.data);bm.free()
camera=scene.camera;camera.location=position(pose['camera']);camera.rotation_euler=(position(pose['target'])-camera.location).to_track_quat('-Z','Y').to_euler();camera.data.type='PERSP';camera.data.sensor_fit='VERTICAL';camera.data.sensor_height=24;camera.data.lens=24/(2*math.tan(math.pi/8));camera.data.shift_y=0
...
scene.render.engine='BLENDER_EEVEE';print('K1_RENDER_ENGINE',scene.render.engine)
